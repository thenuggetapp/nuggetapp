    ...
    try {
      ...
      const [restaurantsResult, citiesResult] = await Promise.all([
        supabase
          .from("restaurants")
          .select(
            "id, name, cuisine, address, image_url, google_place_id, city",
          )
          .eq("visible", true)
          .or(
            `name.ilike.${searchTerm},cuisine.ilike.${searchTerm},address.ilike.${searchTerm}`,
          )
              ...
              <div className="px-4 pb-3">
                ...
                >
                  <div className="flex items-center gap-2">
                    <MapPin
                      className={`h-5 w-5 ${!selectedCity ? "text-red-500" : "text-[#8dbf65]"}`}
                    />
                    <span
                      className={`text-base font-medium ${!selectedCity ? "text-red-600" : "text-slate-900"}`}
                    >
                      {selectedCity || "Select City *"}
                    </span>
                  </div>
                  <div
                    className={`text-xs px-2 py-1 rounded-full ${!selectedCity ? "bg-red-100 text-red-600" : "bg-slate-100 text-slate-600"}`}
                  >
            ...
            <div className="flex-1 overflow-y-auto">
              {showCitySelector && (
                <div className="p-4 bg-slate-50">
                  ...
                  >
                    <Navigation
                      className={`h-5 w-5 text-[#8dbf65] ${isGettingLocation ? "animate-pulse" : ""}`}
                    />
                  ...
                  <div className="space-y-2">
                    {Object.keys(CITIES).map((city) => (
                      ...
                      >
                        <Globe
                          className={`h-5 w-5 ${selectedCity === city ? "text-[#8dbf65]" : "text-slate-400"}`}
                        />
                        <span
                          className={`text-base font-medium ${selectedCity === city ? "text-[#8dbf65]" : "text-slate-900"}`}
                        >
                          {city}
                        </span>
                      </button>
              ...
              {!searchQuery.trim() && !showCitySelector && (
                <div className="p-4">
                  {recentSearches.length > 0 ? (
                    <>
                      <h3 className="text-sm font-semibold text-slate-900 mb-3 text-left">
                        Recent searches
                      </h3>
                      <div className="space-y-2">
              ...
              {searchQuery.trim() && suggestions.length === 0 && (
                <div className="p-4">
                  <div className="flex flex-col items-center justify-center py-12 text-center">
                    ...
                    <p className="text-sm text-slate-500">
                      Try searching for a different restaurant, cuisine, or
                      location
                    </p>
                  </div>
                </div>
