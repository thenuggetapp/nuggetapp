...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (type === 'search') {
      ...
      if (isExactCityMatch && cityCoordinates) {
      ...
      } else if (!parsed?.cuisines?.length && !parsed?.foodKeywords?.length && !Object.keys(parsed?.features || {}).length) {
        ...
        supabaseQuery = supabaseQuery.or(`name.ilike.${searchTerm},description.ilike.${searchTerm},cuisine.ilike.${searchTerm},address.ilike.${searchTerm},city.ilike.${searchTerm}`);
      }
      ...
      if (filterResult.isFiltered) {
        ...
        const { count: totalCount } = await filterResult.query
    ...
    if (type === 'suggestions') {
      ...
      const [restaurantsResult, citiesResult] = await Promise.all([
        supabase
          ...
          .eq('visible', true)
          .or(`name.ilike.${searchTerm},description.ilike.${searchTerm},cuisine.ilike.${searchTerm},address.ilike.${searchTerm}`)
          .order('rating', { ascending: false })
          .limit(5),
        supabase
          .from('restaurants')
      ...
      (citiesResult.data || []).forEach(r => {
        if (r.city) {
          const normalizedCity = r.city.charAt(0).toUpperCase() + r.city.slice(1).toLowerCase();
          cityCountMap.set(normalizedCity, (cityCountMap.get(normalizedCity) || 0) + 1);
        }
      });
